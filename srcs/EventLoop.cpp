    ...
    while (1) {
        ...
        for (int i = 0; i < newEvnts; i++) {
            ...
            if (curEvnts->filter == EVFILT_READ) {
                if (curEvnts->ident < (uintptr_t)servcnt + 3) {
                    int clnt_fd;
...
void EventLoop::HandleRequest(struct kevent *curEvnts) {
    ...
    // std::endl; 	close(curEvnts->ident); 	return ;
    // }
...
void EventLoop::MakeResponse(struct kevent *curEvnts) {
    ...
    //     std::cout << "close fail ㅠㅠ " << std::endl;
    // else
    //     std::cout << "close " << curEvnts->ident << " success⭕️" << std::endl;
    std::cout << "\n\n" << std::endl;
}
